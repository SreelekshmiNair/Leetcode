        ...
        while(left<=right){
           ...
           // Console.WriteLine("Mid is " + mid);
            ...
            //position was position + 1 (since 0 based index).
             ...
             /*If the number of missing integers before mid is less than k, then we update the
             // left pointer to mid + 1; otherwise, we update the right pointer to mid - 1.
             // We continue this process until the left pointer crosses the right pointer, at
             // which point the left pointer will be the index of the first missing integer. 
             //Finally, we add k to the left pointer to get the k-th missing integer.*/
